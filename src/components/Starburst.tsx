...
export function Starburst({
  ...
  return (
    <View style={[{ width: size, height: size, transform: [{ rotate: `${rotate}deg` }] }, style]}>
      ...
      <View style={{ position: 'absolute', top: shadowOffset, left: shadowOffset, width: size, height: size }}>
        <Svg viewBox="0 0 100 100" width="100%" height="100%">
          <Polygon points={path} fill={colors.ink}/>
        </Svg>
      </View>
      {/* main polygon */}
      <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, width: size, height: size } as any}>
        <Svg viewBox="0 0 100 100" width="100%" height="100%">
          <Polygon points={path} fill={fill} stroke={stroke} strokeWidth={strokeWidth} strokeLinejoin="round"/>
        </Svg>
      </View>
      <View style={{
